...
**Why I wrote this.** "How do I get an MLflow model behind a SageMaker endpoint?" is the question I've heard most often since managed MLflow launched — and the honest answer was "it depends" with no side-by-side comparison anywhere. This post is that comparison, with the same model deployed three ways so the trade-offs are visible instead of anecdotal.
{: .notice--info}
